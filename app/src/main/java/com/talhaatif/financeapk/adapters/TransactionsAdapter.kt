...
class TransactionsAdapter(private var groupedTransactions: Map<String, List<Transaction>>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == TYPE_DATE) {
            ...
            TransactionViewHolder(binding, utils)
        }
    }
    ...
    class TransactionViewHolder(private val binding: RvTransactionsBinding, private val utils: Util) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(transaction: Transaction) {
            val context = itemView.context
            val currency = utils.getLocalData(context, "currency") ?: "Rp"

            val tintColor = Variables.categoryTintMap[transaction.category.lowercase()] ?: Color.BLACK
            val categoryImage = Variables.categoryResourceMap[transaction.category.lowercase()] ?: R.drawable.ic_food

            binding.transactionView.setImageResource(categoryImage)
            binding.transactionView.imageTintList = ColorStateList.valueOf(tintColor)

            // Format amount with thousands separator
            val amountDouble = transaction.transAmount.toDoubleOrNull() ?: 0.0
            val formattedAmount = NumberFormat.getNumberInstance(Locale("in", "ID")).format(amountDouble)

            binding.transactionType.text = transaction.category.replaceFirstChar { it.uppercase() }
            
            if (transaction.transType == "Income") {
                binding.transactionAmount.setTextColor(Color.parseColor("#4CAF50")) // Green
                binding.transactionAmount.text = "+ $currency $formattedAmount"
            } else {
                binding.transactionAmount.setTextColor(Color.parseColor("#F44336")) // Red
                binding.transactionAmount.text = "- $currency $formattedAmount"
            }
